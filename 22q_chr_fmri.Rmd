...
packages <- c("devtools","conflicted","here","magrittr", "dplyr", "tidyr","readxl", "ggplot2","ggrepel","ggpubr","RColorBrewer", "ciftiTools","tableone", "data.table", "reshape2","neuroCombat","pls")
...
ahba_pls <- function(ahba=ahbaSurfCABNP_L, map, val_col, verbose=FALSE){
  ...
  pls_test <- plsr(formula=plsformula, data=df, scale=TRUE, center=TRUE, validation="CV", na.action="na.omit", segments=2)
}
...
ahba_permute_pls <- function(map, val_col, bsmash, cols=paste0("V",1:10000), verbose=TRUE){
  ...
  nulls <- lapply(cols, function(c) ahba_pls(map=bsmash, val_col=c,verbose=TRUE))
  # get first component Y R2 from each model
  y_r2 <-  lapply(nulls, function(c) get_yr2) %>% unlist %>% as.vector 
  # return as list 
  out <- list(pls=pls, null_models=nulls, test_stat=y_r2)
  return(out)
}

# test with only a couple permutations  
rsfsa_ahba_perm_pls <- ahba_permute_pls(map=rsfa_22q_lms, val_col="beta", bsmash=rsfa_22q_bsmash_L, cols=paste0("V",1:3))
