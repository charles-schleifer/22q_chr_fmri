...
# exclude Q_0390_09302019 for now due to no AP BOLD, and exclude two NAPLS subjects with no demographic info
exclude_sessions <- c("Q_0390_09302019","01_S0083_00","01_S0128_00")
...
TO DO: multiple things to fix in input data
NetHo data has NAs as well as inf values
...
# TODO: netho failing because of NAs
netho_combat <- run_neurocombat(input=netho_all, var_column="NetHo")
...
plot_combat <- function(df, raw_col){
  ...
  subs <- df$MRI_S_ID
  precombat <- lapply(subs, function(s) mean(data.frame(filter(df, MRI_S_ID==s))[,raw_col])) %>% do.call(rbind,.)
  postcombat <- lapply(subs, function(s) mean(data.frame(filter(df, MRI_S_ID==s))$value)) %>% do.call(rbind,.)
  parc_means <- data.frame(MRI_S_ID=subs, postcombat=as.vector(postcombat), precombat=as.vector(precombat))
  parc_means_demo <- merge(x=parc_means, y=df, by="MRI_S_ID", all.x=TRUE, all.y=FALSE)
