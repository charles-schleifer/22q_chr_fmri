...
print(paste("RSFA CHRc significant regions:",sum(rsfa_chrc_lms$group_FDR_sig)))
...
print(paste("NetHo CHRc significant regions:",sum(netho_chrc_lms$group_FDR_sig)))
...
print(paste("GBC CHRc significant regions:",sum(gbc_chrc_lms$group_FDR_sig)))
...
```
investigate psychosis GBC effect in 22q further
```{r}
test <- lm_parcel_group_covars(df=filter(add_demo_cols(list=gbc_wide, demo=demo_multisite_bl, democols="psych_dx")$df, Group=="22qDel"), var="r_65",predictors="psych_dx + AGE + AGE2 + SEX + Site + percent_udvarsme", groupcol="psych_dx", groups=c("Y","N"), refgroup="N", test_out_model=TRUE) 
