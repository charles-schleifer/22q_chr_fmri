...
ahba_permute_pls <- function(map, val_col, bsmash, cols=paste0("V",1:10000), verbose=FALSE, parallel=TRUE){
  ...
  if(parallel==TRUE){
    # use fewer cores if number of columns is less than the specificed number of cores
    cores=detectCores()
    if(cores > length(cols)){cores=length(cols)}
    null_mods <- mclapply(cols, function(c) ahba_pls(map=bsmash, val_col=c,verbose=FALSE), mc.cores=cores)
    nulls <-  mclapply(null_mods, function(c) get_yr2(input=c, ncomp=1, estimate="CV"), mc.cores=cores) %>% unlist %>% as.vector 
  }else if(parallel==FALSE){
    null_mods <- lapply(cols, function(c) ahba_pls(map=bsmash, val_col=c,verbose=verbose))
    nulls <-  lapply(null_mods, function(c) get_yr2(input=c, ncomp=1, estimate="CV")) %>% unlist %>% as.vector 
  }
  ...
  # don't return models to save memory
  out <- list(test_stat=test_stat, nulls=nulls, pval=p)
  return(out)
}
