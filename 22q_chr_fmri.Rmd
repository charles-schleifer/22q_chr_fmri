...
ahba_pls <- function(ahba=ahbaSurfCABNP_L, map, val_col){
  indices <- ahba$INDEX
  # get list of genes
  ...
  # test model
  pls_test <- plsr(formula=plsformula, data=df, scale=TRUE, center=TRUE, validation="CV", na.action="na.omit")
}
...
y_r2 <- R2(rsfsa_ahba_pls, estimate = "train", intercept = FALSE)$val %>% drop 
x_r2 <- cumsum(explvar(rsfsa_ahba_pls))/100
