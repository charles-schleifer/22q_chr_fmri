...
run_neurocombat <- function(input, 
                            ...
                            covars=c("MRI_S_ID","AGE","AGE2","SEX","Site","percent_udvarsme"), 
                            index_col="INDEX", 
                            id_col="MRI_S_ID", 
                            demo=demo_22q_napls){
  ...
  covardf <- merge(x=data.frame(subnames), y=demo[,covars], by.x="subnames", by.y=id_col, all.x=TRUE)
  ...
  model_matrix <- model.matrix(~covardf$AGE + covardf$SEX + covardf$percent_udvarsme)
  combat <- neuroCombat(dat=input_subcols[,subcols], batch=covardf$Site, mod=model_matrix, parametric=TRUE, eb=TRUE)
  
  # convert combat results  back to long df
  combat_d <- combat$dat.combat %>% as.data.frame
  data.table::setDT(combat_d)
  combat_d[,index_col] <- rownames(combat_d)
  combat_d[,index_col] %<>% as.numeric
  combat_l <- melt.data.table(combat_d, id.vars=index_col) %>% rename(id_col="variable")
  
  # merge with demographics
  out <- merge(x=input, y=combat_l, by=c(index_col, id_col), all.x=TRUE)
  return(out)
}

test <- run_neurocombat(input=gbc_all, var_column="GBC")
