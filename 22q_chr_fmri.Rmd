...
rsfa_22q_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("RSFA 22q significant regions:",sum(rsfa_22q_lms$FDR_sig)))

rsfa_chr_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("RSFA CHR significant regions:",sum(rsfa_chr_lms$FDR_sig)))

netho_22q_lms <- get_parcel_group_lm(list=netho_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("NetHo 22q significant regions:",sum(netho_22q_lms$FDR_sig, na.rm=TRUE)))

netho_chr_lms <- get_parcel_group_lm(list=netho_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("NetHo CHR significant regions:",sum(netho_chr_lms$FDR_sig, na.rm=TRUE)))

gbc_22q_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("GBC 22q significant regions:",sum(gbc_22q_lms$FDR_sig)))

gbc_chr_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("GBC CHR significant regions:",sum(gbc_chr_lms$FDR_sig)))

```
test CHRc vs TD
```{r}
rsfa_chrc_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("RSFA CHRc significant regions:",sum(rsfa_chrc_lms$FDR_sig)))

netho_chrc_lms <- get_parcel_group_lm(list=netho_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
print(paste("NetHo CHRc significant regions:",sum(netho_chrc_lms$FDR_sig)))

gbc_chrc_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N",roi_key=filter(ji_key,INDEX %in% 1:360))
...
cor_nulls1 <- plot_cor_nulls(test_stat=cor_stats[[which(maps_all_df$Var1 == "22q LC" & maps_all_df$Var2 == "22q BSV")]]$true_correlation, nulls=cor_stats[[which(maps_all_df$Var1 == "22q LC" & maps_all_df$Var2 == "22q BSV")]]$nulls, xlab="Pearson r", title="22qDel LC vs. 22qDel BSV")

cor_nulls2 <- plot_cor_nulls(test_stat=cor_stats[[which(maps_all_df$Var1 == "22q GBC" & maps_all_df$Var2 == "CHR LC")]]$true_correlation, nulls=cor_stats[[which(maps_all_df$Var1 == "22q GBC" & maps_all_df$Var2 == "CHR LC")]]$nulls, xlab="Pearson r", title="22qDel GBC vs. CHR LC")

cor_nulls_all <- cor_nulls1+cor_nulls2+ 
  plot_annotation(title = 'Null distributions for significant comparisons',theme=theme(plot.title=element_text(size=15,hjust=0.5)))
cor_nulls_all
...
cabnp_hcp_lh <- function(hcp_df, colname="GlasserRegion", cabnp_df, ahba_df=ahbaSurfPVSST_L, ahba_cols="PVALB_SST"){
...
nmap_match <- data.frame(variable=c("cmruglu", "cmr02", "CBF", "cbv", "PC1.AHBA", "PVALB_SST", "G1.fMRI", "intersubjvar", "PC1.Neurosynth", "scalingpnc", "scalingnih", "devexp", "Evolution.Expansion", "Cortical.Thickness", "T1T2ratio", "meg_delta", "meg_theta", "meg_alpha", "meg_beta", "meg_gamma1", "meg_gamma2"), name=c("Glucose metabolism", "Oxygen metabolism", "Cerebral blood flow", "Cerebral blood volume", "PC1 gene expression", "PVALB-SST gradient","fMRI gradient", "Intersubject variability", "PC1 neurosynth", "Allometric scaling (PNC)", "Allometric scaling (NIH)", "Developmental expansion", "Evolutionary expansion", "Cortical thickness", "T1w/T2w ratio", "MEG delta", "MEG theta", "MEG alpha", "MEG beta", "MEG gamma1", "MEG gamma2"))
...
demo_table_c <- CreateTableOne(data=demo_22q_napls, strata="GroupConvert", vars=c("AGE","SEX"))
demo_table_c
```
