...
packages <- c("devtools","conflicted","here","magrittr", "dplyr", "tidyr","readxl", "ggplot2","patchwork", "ggrepel", "ggpubr", "RColorBrewer", "ciftiTools", "tableone", "data.table", "reshape2", "neuroCombat", "pls", "parallel")
...
# run combat
rsfa_combat_22q <- run_neurocombat(input=filter(rsfa_all, site %in% c("UCLAtrio","UCLAprisma","SUNY","Rome","IoP" )), var_column="t_sd")
rsfa_combat_napls <- run_neurocombat(input=filter(rsfa_all, site=="NAPLS2"), var_column="t_sd")
#rsfa_combat <- rbind(rsfa_combat_22q,rsfa_combat_napls)

netho_combat_22q <- run_neurocombat(input=filter(netho_no_na, site %in% c("UCLAtrio","UCLAprisma","SUNY","Rome","IoP" )), var_column="NetHo")
netho_combat_napls <- run_neurocombat(input=filter(netho_no_na, site=="NAPLS2"), var_column="NetHo")
#netho_combat <- rbind(netho_combat_22q,netho_combat_napls)

gbc_combat_22q <- run_neurocombat(input=filter(gbc_all, site %in% c("UCLAtrio","UCLAprisma","SUNY","Rome","IoP" )), var_column="GBC")
gbc_combat_napls <- run_neurocombat(input=filter(gbc_all, site=="NAPLS2"), var_column="GBC")
...
rsfa_22q_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL")
print(paste("RSFA 22q significant regions:",sum(rsfa_22q_lms$FDR_sig)))

rsfa_chr_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N")
print(paste("RSFA CHR significant regions:",sum(rsfa_chr_lms$FDR_sig)))

netho_22q_lms <- get_parcel_group_lm(list=netho_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL")
print(paste("NetHo 22q significant regions:",sum(netho_22q_lms$FDR_sig, na.rm=TRUE)))

netho_chr_lms <- get_parcel_group_lm(list=netho_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N")
print(paste("NetHo CHR significant regions:",sum(netho_chr_lms$FDR_sig, na.rm=TRUE)))

gbc_22q_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("22qDel","CONTROL"),refgroup="CONTROL")
print(paste("GBC 22q significant regions:",sum(gbc_22q_lms$FDR_sig)))

gbc_chr_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("CHR","CONTROL-N"),refgroup="CONTROL-N")
print(paste("GBC CHR significant regions:",sum(gbc_chr_lms$FDR_sig)))

```
test CHRc vs TD
```{r}
rsfa_chrc_lms <- get_parcel_group_lm(list=rsfa_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N")
print(paste("RSFA CHRc significant regions:",sum(rsfa_chrc_lms$FDR_sig)))

netho_chrc_lms <- get_parcel_group_lm(list=netho_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N")
print(paste("NetHo CHRc significant regions:",sum(netho_chrc_lms$FDR_sig)))

gbc_chrc_lms <- get_parcel_group_lm(list=gbc_wide, groups=c("CHRc","CONTROL-N"), groupcol="GroupConvert",refgroup="CONTROL-N")
...
# 22q NetHo
pl_netho_22q_b <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=netho_22q_lms, roi_col="INDEX", val_col="beta")
view_xifti_surface(pl_netho_22q_b, title="Local Connectivity: 22qDel vs TD", cex.title=1.3,zlim=c(-0.8,0.8), colors=pal_red_blue())

# 22q GBC
pl_gbc_22q_b <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=gbc_22q_lms, roi_col="INDEX", val_col="beta")
...
# chr NetHo
pl_netho_chr_b_fdr <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=netho_chr_lms, roi_col="INDEX", val_col="beta_FDR")
...
plot_cor_nulls <- function(test_stat, nulls, alternative="two.sided", title="", xlab="test statistic"){
  ...
  ggplot(data=data.frame(nulls=nulls), aes(x=nulls))+
    ...
    geom_label_repel(data=data.frame(xval=test_stat, text=paste("p =",round(p, digits=3))), aes(x=xval, y=0, label=text), min.segment.length = 0, nudge_y=100, nudge_x=-0.05)+
    xlab(xlab)+
    ggtitle(title)+
    theme_classic()
}
...
plot_cor_nulls(test_stat=netho_rsfa_22q_cor_L$true_correlation, nulls=netho_rsfa_22q_cor_L$nulls, xlab="Pearson r", title="NetHo vs RSFA, LH")
...
cabnp_hcp_lh <- function(hcp_df, colname="GlasserRegion", cabnp_df){
...
perm_p_neuromaps <- function(neuromap_outputs, order=nmap_order, neuromap_df=neuromaps_ordered, bsmash, bsmash_cols=paste0("V",1:10000), alternative="two.sided", verbose=FALSE){
...
nmap_order <- c("cmruglu","CBF","cbv","cmr02","G1.fMRI","intersubjvar","PC1.AHBA","PC1.Neurosynth","scalingpnc","scalingnih","devexp","Evolution.Expansion","Cortical.Thickness","T1T2ratio","meg_delta","meg_theta","meg_alpha","meg_beta","meg_gamma1","meg_gamma2")
...
rsfa_22q_nuls_long$variable <- factor(rsfa_22q_nuls_long$variable,levels=nmap_order,ordered=TRUE)

netho_22q_nuls_long <- melt.data.table(as.data.table(netho_22q_nmap_p$nulls), id.vars="perm")
netho_22q_nuls_long$variable <- factor(netho_22q_nuls_long$variable,levels=nmap_order,ordered=TRUE)

gbc_22q_nuls_long <- melt.data.table(as.data.table(gbc_22q_nmap_p$nulls), id.vars="perm")
gbc_22q_nuls_long$variable <- factor(gbc_22q_nuls_long$variable,levels=nmap_order,ordered=TRUE)
...
neuromaps_22q_bplots <- rsfa_box/netho_box/gbc_box+plot_layout(guides = "collect")
