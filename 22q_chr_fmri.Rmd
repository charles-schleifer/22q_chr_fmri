...
#}
lm_parcel_group_covars <- function(df,var,predictors){
  lm(reformulate(response=var),data=df)
}

# function to apply lm at each parcel, expects list object output from rows_to_cols, default formula set in function but different predictors can be specified
get_parcel_group_lm <- function(list, predictors="Group + AGE + AGE2 + SEX + Site + percent_udvarsme"){
  df <- list$df
  parc_cols <- list$parcels
  lapply(parc_cols, function(v) lm_parcel_group_covars(var=v, df=df, predictors=predictors))
}
