...
perm_p_neuromaps <- function(neuromap_outputs, neuromap_df=neuromaps_ordered, bsmash, bsmash_cols=paste0("V",1:10000), alternative="two.sided", verbose=FALSE){
  maps <- neuromap_outputs$map
  for(map in maps){
    # get true correlation
    cor <- filter(neuromap_outputs, map==map)
    # get nulls
    nulls <- map_permute_cor(map=neuromap_df, variable=map, bsmash=bsmash, bsmash_cols=bsmash_cols)
    # get p-val
    p <- perm_p(test_stat=cor, nulls=nulls, alternative=alternative)
    # return output
    out <- list(cor=cor, nulls=nulls, p=p)
    return(out)
  }
}
