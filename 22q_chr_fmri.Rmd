...
packages <- c("devtools","conflicted","here","magrittr", "dplyr", "tidyr","readxl", "ggplot2", "ggrepel", "ggpubr", "RColorBrewer", "ciftiTools", "tableone", "data.table", "reshape2", "neuroCombat", "pls", "parallel")
...
rsfa_ahba_cor <- cor_brain_ahba_all(map=rsfa_22q_lms, verbose=TRUE)
...
```{r}
# calculates 1-tailed p-value based on how many test stats from the null distribution are greater than the true test stat
ahba_permute_cor <- function(ahba=ahbaSurfCABNP_L, gene, bsmash, bsmash_cols=paste0("V",1:10000), verbose=FALSE, method="pearson", use="complete.obs"){
  if(verbose==TRUE){print(gene)}
  # error if bsmash and ahba have overlapping column names
  if(sum(names(bsmash) %in% names(ahba) & names(bsmash) != "INDEX") != 0){stop("Error: bsmash and ahba have overlapping column names")} 
  indices <- ahba$INDEX
  # compute permuted test stats
  nulls <- NULL
  for(i in 1:length(bsmash_cols)){
    df <- merge(x=ahba, y=bsmash, by="INDEX")
    col <- bsmash_cols[i]
    # print message on the same line for each loop
    if(verbose==TRUE){
    ...
    null <- cor(x=as.numeric(df[,gene]), y=as.numeric(df[,col]), use=use, method=method) %>% as.numeric
    nulls <- c(nulls, null)
  }
  return(nulls)
}
...
rsfa_ahba_abs95_nulls <- lapply(rsfa_ahba_abs95, function(g) ahba_permute_cor(gene=g, bsmash=rsfa_22q_bsmash_L, verbose=TRUE))
```
