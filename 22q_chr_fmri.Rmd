...
perm_p <- function(input){
  sum <- sum(abs(input$nulls) > abs(input$true_correlation))
  len <- length(input$nulls)
  out <- sum/len
  print(sum)
  print(len)
  print(out)
...
plot_cor_nulls <- function(input, title="true correlation vs nulls"){
  p <- perm_p(input)
  ggplot(data=data.frame(nulls=input$nulls), aes(x=nulls))+
    geom_histogram(bins=40,fill="slategrey", alpha=0.7)+
    geom_vline(xintercept=input$true_correlation, color="red", size=1, lty="solid")+
    geom_label_repel(data=data.frame(xval=input$true_correlation, text=paste("p =",round(p, digits=3))), aes(x=xval, y=0, label=text), min.segment.length = 0, nudge_y=100, nudge_x=-0.05)+
    xlab("correlation")+
    ggtitle(title)+
    theme_classic()
}
...
plot_cor_nulls(netho_rsfa_22q_cor_L, title="NetHo vs RSFA, LH")
plot_cor_nulls(netho_rsfa_22q_cor_R, title="NetHo vs RSFA, RH")
...
plot_cor_nulls(gbc_rsfa_22q_cor_L, title="GBC vs RSFA, LH")
plot_cor_nulls(gbc_rsfa_22q_cor_R, title="GBC vs RSFA, RH")
plot_cor_nulls(gbc_netho_22q_cor_L, title="GBC vs NetHo, LH")
plot_cor_nulls(gbc_netho_22q_cor_R, title="GBC vs NetHo, RH")
...
#y_r2 <- get_yr2(input=rsfsa_ahba_pls, ncomp=1:5)
#x_r2 <- cumsum(explvar(rsfsa_ahba_pls))[1:5]/100
...
# calculates 1-tailed p-value based on the fraction of permuted test stats greater than the true test stat
ahba_permute_pls <- function(map, val_col, bsmash, cols=paste0("V",1:10000), verbose=FALSE, parallel=TRUE, cores=8){
  ...
  # test same model for permuted maps
  if(parallel==TRUE){
    null_mods <- mclapply(cols, function(c) ahba_pls(map=bsmash, val_col=c,verbose=FALSE), mc.cores=cores)
    nulls <-  mclapply(nulls, function(c) get_yr2, mc.cores=cores) %>% unlist %>% as.vector 
  }else if(parallel==FALSE){
    null_mods <- lapply(cols, function(c) ahba_pls(map=bsmash, val_col=c,verbose=verbose))
    nulls <-  lapply(nulls, function(c) get_yr2(input=c, ncomp=1, estimate="CV")) %>% unlist %>% as.vector 
  }
  # get 1-tailed p-value
  p <- sum(nulls > test_stat)/length(nulls)
  # return as list with original pls model, test statistic (first component Y R2), all null models, null test statistics, and 1-tailed p-value 
  out <- list(pls=pls, test_stat=test_stat, null_models=null_mods, nulls=nulls, pval=p)
  return(out)
}

# test with only a couple permutations  
rsfsa_ahba_perm_test <- ahba_permute_pls(map=rsfa_22q_lms, val_col="beta", bsmash=rsfa_22q_bsmash_L, cols=paste0("V",1:2), parallel=TRUE)

# RSFA PLS permutation
#rsfsa_ahba_perm <- ahba_permute_pls(map=rsfa_22q_lms, val_col="beta", bsmash=rsfa_22q_bsmash_L, cols=paste0("V",1:10000), parallel=TRUE)
