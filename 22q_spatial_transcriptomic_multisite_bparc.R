...
# also reads gbc values calculated by (which script?) and tests imaging transcriptomic correlations
...
####################################################################################################################################################
### compute gbc difference z score in 22qDel vs HCS sample
####################################################################################################################################################
...
# for now also exclude several new sessions without gbc calculated
...
# read bparc
#bparc_22q <- lapply(use_ids_22q, function(s) read_csv_results(sesh=s,site="trio",sdir=file.path(hoffman,"22q/qunex_studyfolder/sessions"),fname=bparc_name)) 
#bparc_hcs <- lapply(use_ids_hcs, function(s) read_csv_results(sesh=s,site="trio",sdir=file.path(hoffman,"22q/qunex_studyfolder/sessions"),fname=bparc_name)) 

trio_bparc <- lapply(filter(demo_multisite_bl, Site=="UCLAtrio")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="UCLAtrio",sdir=trio_dir,fname=bparc_name))
prisma_bparc <- lapply(filter(demo_multisite_bl, Site=="UCLAprisma")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="UCLAprisma",sdir=prisma_dir,fname=bparc_name_prisma)) 
suny_bparc <- lapply(filter(demo_multisite_bl, Site=="SUNY")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="SUNY",sdir=suny_dir,fname=bparc_name)) 
rome_bparc <- lapply(filter(demo_multisite_bl, Site=="Rome")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="Rome",sdir=rome_dir,fname=bparc_name)) 
iop_bparc <- lapply(filter(demo_multisite_bl, Site=="IoP")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="IoP",sdir=iop_dir,fname=bparc_name)) 


# function to get average connectivity (gbc) for each roi from single subject FC matrix
get_gbc <- function(df){
  # make data frame with row for each 718 parcels
  gbc <- data.frame(INDEX=1:718)
  gbc[,"MRI_S_ID"] <- df$MRI_S_ID %>% unique
  gbc[,"site"] <- df$site %>% unique
  # for each parcel, get mean FC for all connections involving parcel
  for(i in 1:718){
    gbc[i,"GBC"] <- filter(df, roi_1==i | roi_2==i)$pearson_r_Fz %>% as.numeric %>% mean
  }
  return(gbc)
}

trio_gbc <- lapply(trio_bparc, get_gbc) %>% do.call(rbind,.)
prisma_gbc <- lapply(prisma_bparc, get_gbc) %>% do.call(rbind,.)
suny_gbc <- lapply(suny_bparc, get_gbc) %>% do.call(rbind,.)
rome_gbc <- lapply(rome_bparc, get_gbc) %>% do.call(rbind,.)
iop_gbc <- lapply(iop_bparc, get_gbc) %>% do.call(rbind,.)

gbc_all <- rbind(trio_gbc,prisma_gbc,suny_gbc,rome_gbc,iop_gbc)

# save gbc_all
write.csv(gbc_all, file=file.path(project,"gbc_all.csv"), row.names=FALSE)

# read gbc_all
gbc_all <- read.csv(file=file.path(project,"gbc_all.csv"))
...
data.table::setDT(gbc_all)
gbc_all_subcols <- reshape2::dcast(data=gbc_all, formula="INDEX ~ MRI_S_ID", value.var="GBC") 

# get subject ids in order of columns
subcols <- which(names(gbc_all_subcols)!= "INDEX")
subnames <- names(gbc_all_subcols)[subcols]
...
# run neurocombat for parcellated gbc with site as batch variable and AGE and SEX included in the model
...
gbc_all_combat <- neuroCombat(dat=gbc_all_subcols[,subcols], batch=combat_covars_ordered$Site, mod=model.matrix(~combat_covars_ordered$AGE+combat_covars_ordered$SEX+combat_covars_ordered$percent_udvarsme), parametric=TRUE)

# get mean of all parcels per subject pre/post combat to look at distributions
parc_means_postcombat <- apply(gbc_all_combat$dat.combat,  MARGIN=2, FUN="mean")
parc_means_precombat <- apply(gbc_all_combat$dat.original,  MARGIN=2, FUN="mean")
...
# plot meean gbc distributions by site pre combat
pl_precombat <- ggplot(parc_means_demo, aes(precombat, fill=Site, y=..count..))+
  geom_density(kernel="gaussian", alpha=0.7)+
  #scale_fill_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("Mean GBC pre-combat")

# plot meean gbc distributions by site post combat
pl_postcombat <- ggplot(parc_means_demo, aes(postcombat, fill=Site, y=..count..))+
  geom_density(kernel="gaussian", alpha=0.7)+
  #scale_fill_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("Mean GBC post-combat")

ggarrange(pl_precombat,pl_postcombat, nrow=1, common.legend=T,legend="right")

# convert combat results  back to long df
gbc_all_combat_d <- gbc_all_combat$dat.combat %>% as.data.frame
data.table::setDT(gbc_all_combat_d)
gbc_all_combat_d$INDEX <- rownames(gbc_all_combat_d)
gbc_all_combat_d$INDEX %<>% as.numeric
gbc_all_combat_l <- melt.data.table(gbc_all_combat_d, id.vars="INDEX") %>% rename("MRI_S_ID"="variable")  %>% rename("GBC.combat"="value")

# merge with original gbc_all
gbc_all_combat_l_full <- merge(x=gbc_all, y=gbc_all_combat_l, by=c("INDEX","MRI_S_ID"))

## as a check, do the same for data.original from combat output. should be the same as t_sd
#gbc_all_original_d <- gbc_all_combat$dat.original %>% as.data.frame
#data.table::setDT(gbc_all_original_d)
#gbc_all_original_d$INDEX <- rownames(gbc_all_original_d)
#gbc_all_original_d$INDEX %<>% as.numeric
#gbc_all_original_l <- melt.data.table(gbc_all_original_d, id.vars="INDEX") %>% rename("MRI_S_ID"="variable")  %>% rename("t_sd_original"="value")
## merge with original gbc_all
#gbc_all_combat_l_full_check <- merge(x=gbc_all_combat_l_full, y=gbc_all_original_l, by=c("INDEX","MRI_S_ID"), all.x=TRUE)
## check that all match
#all(gbc_all_combat_l_full_check$t_sd == gbc_all_combat_l_full_check$t_sd_original)
...
get_parc_combat_hc_mean_sd <- function(df, hcs_ids, pt_ids, parc){
  ...
  out <- data.frame(parcel_hc_mean=mean(as.numeric(dfh$GBC.combat)), parcel_hc_sd=sd(as.numeric(dfh$GBC.combat)),parcel_pt_mean=mean(as.numeric(dfp$GBC.combat)), parcel_pt_sd=sd(as.numeric(dfp$GBC.combat)))
  return(out)
}
...
#parc_hc_stats <- lapply(unique(gbc_all_combat_l_full$INDEX), function(p) get_parc_combat_hc_mean_sd(parc=p, df=gbc_all_combat_l_full, hcs_ids=use_ids_hcs)) 
#parc_hc_stats <- lapply(unique(gbc_all_combat_l_full$INDEX), function(p) get_parc_combat_hc_mean_sd(parc=p, df=gbc_all_combat_l_full, hcs_ids=use_ids_hcs)) %>% do.call(rbind,.)
parc_hc_stats <- lapply(unique(gbc_all_combat_l_full$INDEX), function(p) get_parc_combat_hc_mean_sd(parc=p, df=gbc_all_combat_l_full, hcs_ids=use_ids_hcs, pt_ids=use_ids_del)) %>% do.call(rbind,.)
parc_hc_stats$INDEX <- unique(gbc_all_combat_l_full$INDEX)

# merge control parcel stats with data
gbc_normed <- merge(x=gbc_all_combat_l_full, y=parc_hc_stats, by="INDEX", all.x=TRUE)

# normalize data in each parcel based on control mean and sd for that parcel
gbc_normed$GBC.normed <- (gbc_normed$GBC.combat - gbc_normed$parcel_hc_mean)/gbc_normed$parcel_hc_sd

# cast to wide to merge with demographics
data.table::setDT(gbc_normed)
# add prefix "r_" to parcel indices to use as column names in wide df 
gbc_normed$index_col <- paste0("r_",gbc_normed$INDEX)
# get list of indices
parc_cols <- unique(gbc_normed$index_col)
# make wide df with column form MRI_S_ID and one column per parcel with cells containing gbc.normed
gbc_normed_wide <- reshape2::dcast(gbc_normed, MRI_S_ID ~ index_col, value.var="GBC.normed") 
# order cols
gbc_normed_wide <- gbc_normed_wide[,c("MRI_S_ID", parc_cols)]

## test without normalization
## cast to wide to merge with demographics
#data.table::setDT(gbc_all_combat_l_full)
## add prefix "r_" to parcel indices to use as column names in wide df 
#gbc_all_combat_l_full$index_col <- paste0("r_",gbc_all_combat_l_full$INDEX)
## get list of indices
#parc_cols <- unique(gbc_all_combat_l_full$index_col)
## make wide df with column form MRI_S_ID and one column per parcel with cells containing gbc.normed
#gbc_all_combat_wide <- reshape2::dcast(gbc_all_combat_l_full, MRI_S_ID ~ index_col, value.var="gbc.combat") 
## order cols
#gbc_all_combat_wide <- gbc_all_combat_wide[,c("MRI_S_ID", parc_cols)]

# merge with demo table 
df_demo_table_full_mri <- merge(x=demo_multisite_bl, y=gbc_normed_wide, by="MRI_S_ID")
#df_demo_table_full_mri <- merge(x=demo_multisite_bl, y=gbc_all_combat_wide, by="MRI_S_ID")
...
#gbc_group_betas <- get_parcel_group_b_lm(df=df_demo_table_full_mri, parc_cols=parc_cols)%>% do.call(rbind,.) %>% as.data.frame
gbc_group_lms <- get_parcel_group_lm(df=df_demo_table_full_mri, parc_cols=parc_cols)
gbc_group_betas <- lapply(gbc_group_lms, function(m)m$coefficients["SUBJECT_IDENTITYPATIENT-DEL"]) %>% do.call(rbind,.) %>% as.vector
gbc_group_ts <- lapply(gbc_group_lms, function(m) summary(m)$coefficients["SUBJECT_IDENTITYPATIENT-DEL","t value"]) %>% do.call(rbind,.) %>% as.vector
gbc_group_ps <- lapply(gbc_group_lms, function(m) summary(m)$coefficients["SUBJECT_IDENTITYPATIENT-DEL","Pr(>|t|)"]) %>% do.call(rbind,.) %>% as.vector
gbc_group_fdrp <- p.adjust(gbc_group_ps, method="fdr")
...
ji_key$gbc_diff_beta <- gbc_group_betas
# add t- p- and q-values to cab-np parcel key
ji_key$gbc_diff_t <- gbc_group_ts
ji_key$gbc_diff_para_p <- gbc_group_ps
ji_key$gbc_diff_para_q <- gbc_group_fdrp
# create column of betas with non-fdr-significant set to NA
ji_key$gbc_diff_beta_parasig <- ji_key$gbc_diff_beta
ji_key$gbc_diff_beta_parasig[which(ji_key$gbc_diff_para_q > 0.05)] <- NA
...
#write.csv(ji_key_out, file=file.path(project,"cabnp_key_gbc.csv"),row.names = FALSE)

# create dataframe for input to atlas_xifti_new_vals() with label column (roi_col) and gbc outputs (val_col)
#gbc_bg <- cbind(1:length(gbc_diff),gbc_diff,gbc_delta) %>% as.data.frame
#colnames(gbc_bg) <- c("label","diff","delta")
...
plot_gbc_diff_beta <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=ji_key, roi_col="INDEX", val_col="gbc_diff_beta")
view_xifti_surface(plot_gbc_diff_beta, title="gbc diff beta (22qDel - HCS)", cex.title=1.3,zlim=c(-0.6,0.6),colors=pal_red_blue())
view_xifti_volume(plot_gbc_diff_beta, title="gbc diff beta (22qDel - HCS)", cex.title=1.3,zlim=c(-0.6,0.6),colors=pal_red_blue())

# group difference beta only fdr<0.05
plot_gbc_diff_beta_fdr <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=ji_key, roi_col="INDEX", val_col="gbc_diff_beta_parasig")
view_xifti_surface(plot_gbc_diff_beta_fdr, title="gbc diff beta FDR corrected (22qDel - HCS)", cex.title=1.3,zlim=c(-0.6,0.6), colors=pal_red_blue())
view_xifti_volume(plot_gbc_diff_beta_fdr, title="gbc diff", cex.title=1.3,zlim=c(-0.8,0.8), colors=pal_red_blue(),crop=FALSE, orientation_labels=FALSE, slices=seq(16,42, by=5))

# save fdr corrected maps 
view_xifti_surface(plot_gbc_diff_beta_fdr, title="gbc diff beta FDR corrected (22qDel - HCS)", cex.title=1.3,zlim=c(-0.6,0.6), colors=pal_red_blue(), fname=file.path(project,"figures/gbc/gbc_fdr_surf.png"), width=1200, height=800)
view_xifti_volume(plot_gbc_diff_beta_fdr, title="gbc diff", cex.title=1.3,zlim=c(-0.8,0.8),slices=seq(16,42, by=5), colors=pal_red_blue(),crop=FALSE, orientation_labels=FALSE, legend_embed=FALSE, fname=file.path(project,"figures/gbc/gbc_fdr_vol.pdf"), width=1500, height=2000, format="pdf")

# control means
plot_gbc_hc <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=ji_key, roi_col="INDEX", val_col="parcel_hc_mean")
view_xifti_surface(plot_gbc_hc, title="gbc Control Means", zlim=c(0,25),cex.title=1.3, colors="magma")

# 22qdel means
plot_gbc_del <- atlas_xifti_new_vals(xii=xii_Ji_parcel, df=ji_key, roi_col="INDEX", val_col="parcel_pt_mean")
view_xifti_surface(plot_gbc_del, title="gbc 22qDel Means", zlim=c(0,25),cex.title=1.3, colors="magma")
...
filter(ji_key, !is.na(gbc_diff_beta_parasig)) %>% nrow
# decreases network summary cortex
filter(ji_key[1:360,], gbc_diff_beta_parasig < 0)$NETWORK %>% as.factor %>% summary
# decreases network summary subcortex
filter(ji_key[360:718,], gbc_diff_beta_parasig < 0)$NETWORK %>% as.factor %>% summary
...
gbc_ahba <- merge(x=ji_key, y=ahbaCombinedCABNP, by.x="INDEX", by.y="label")

# get set of usable parcels from left cortex by excluding rows where a test gene (e.g. PVALB) is NA
#gbc_lh_ahba <- filter(gbc_ahba, INDEX <= 180 & !is.na(PVALB))
gbc_lh_ahba <- filter(gbc_ahba, INDEX <= 180)

# correlate delta with PVALB, SST, and PVALB-SST diff
cor_lh_pv <- cor.test(gbc_lh_ahba$gbc_diff_beta, gbc_lh_ahba$PVALB, na.action="omit")
cor_lh_st <- cor.test(gbc_lh_ahba$gbc_diff_beta, gbc_lh_ahba$SST, na.action="omit")
cor_lh_pv_sst_diff <- cor.test(gbc_lh_ahba$gbc_diff_beta, gbc_lh_ahba$PVALB_SST_diff, na.action="omit")
...
cor_hcmean_pv <- cor.test(gbc_lh_ahba$parcel_hc_mean, gbc_lh_ahba$PVALB, na.action="omit")
cor_delmean_pv <- cor.test(gbc_lh_ahba$parcel_pt_mean, gbc_lh_ahba$PVALB, na.action="omit")
cor_hcmean_pv
cor_delmean_pv

# correlate group means with SST
cor_hcmean_st <- cor.test(gbc_lh_ahba$parcel_hc_mean, gbc_lh_ahba$SST, na.action="omit")
cor_delmean_st <- cor.test(gbc_lh_ahba$parcel_pt_mean, gbc_lh_ahba$SST, na.action="omit")
cor_hcmean_st
cor_delmean_st
...
#gene_cor(df=gbc_lh_ahba, mricol="gbc_diff_beta", gene="PVALB")
#gene_cor_list(df=gbc_lh_ahba, mricol="gbc_diff_beta", genelist=filter(zcells_use_final, CellType=="Interneuron")$GeneSymbol) %>% do.call(rbind,.) %>% as.vector

zcell_gbc_distributions <- lapply(zcells_types, function(t) gene_cor_list(df=gbc_lh_ahba, mricol="gbc_diff_beta", genelist=filter(zcells_use_final, CellType==t)$GeneSymbol))
names(zcell_gbc_distributions) <- zcells_types
...
zcell_gbc_plots <- lapply(zcells_types, function(t) plot_dist(vec=unlist(zcell_gbc_distributions[[t]]), title=t))
...
# get indices of regions with significantly decreased gbc
ji_key_neg_gbc <- filter(ji_key, gbc_diff_beta_parasig < 0 )
neg_gbc_parcels <- ji_key_neg_gbc$INDEX
neg_gbc_parces_colnames <- paste0("r_",neg_gbc_parcels)

# get column of the mean normed value of the rois with significant negative 22qDel effect for each subject
df_demo_table_ucla_mri_beh$mean_sig_decrease_rois <- apply(df_demo_table_ucla_mri_beh[,neg_gbc_parces_colnames],1,mean)

# subset to only 22qDel
df_demo_table_ucla22q_mri_beh <- filter(df_demo_table_ucla_mri_beh, SUBJECT_IDENTITY=="PATIENT-DEL")

# correlate composite negative gbc value with SIPS
...
# TODO: in 22q, lm(parcel_gbc ~ SIPS_p)
