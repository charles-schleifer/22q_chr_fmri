...
trio_bparc <- lapply(filter(demo_multisite_bl, Site=="UCLAtrio")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="UCLAtrio",sdir=trio_dir,fname=bparc_name)) %>% do.call(rbind,.)
prisma_bparc <- lapply(filter(demo_multisite_bl, Site=="UCLAprisma")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="UCLAprisma",sdir=prisma_dir,fname=bparc_name_prisma)) %>% do.call(rbind,.)
suny_bparc <- lapply(filter(demo_multisite_bl, Site=="SUNY")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="SUNY",sdir=suny_dir,fname=bparc_name)) %>% do.call(rbind,.)
rome_bparc <- lapply(filter(demo_multisite_bl, Site=="Rome")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="Rome",sdir=rome_dir,fname=bparc_name)) %>% do.call(rbind,.)
iop_bparc <- lapply(filter(demo_multisite_bl, Site=="IoP")$MRI_S_ID, function(s) read_csv_results(sesh=s,site="IoP",sdir=iop_dir,fname=bparc_name)) %>% do.call(rbind,.)

bparc_all <- rbind(trio_bparc,prisma_bparc,suny_bparc,rome_bparc,iop_bparc)
bparc_all$INDEX %<>% as.numeric
