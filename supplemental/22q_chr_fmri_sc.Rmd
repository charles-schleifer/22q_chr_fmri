...
packages <- c("devtools","conflicted","here","magrittr", "dplyr", "tidyr","readxl", "ggplot2","gghalves","patchwork", "ggrepel", "ggpubr", "RColorBrewer", "ciftiTools", "tableone", "data.table", "reshape2", "neuroCombat", "pls", "parallel")
...
nmap_match <- data.frame(variable=c("cmruglu","cmr02","CBF","cbv","PC1.AHBA","G1.fMRI","intersubjvar","PC1.Neurosynth","scalingpnc","scalingnih","devexp","Evolution.Expansion","Cortical.Thickness","T1T2ratio","meg_delta","meg_theta","meg_alpha","meg_beta","meg_gamma1","meg_gamma2"),name=c("Glucose metabolism","Oxygen metabolism", "Cerebral blood flow", "Cerebral blood volume", "PC1 gene expression", "fMRI gradient", "Intersubject variability", "PC1 neurosynth", "Allometric scaling (PNC)", "Allometric scaling (NIH)", "Developmental expansion", "Evolutionary expansion", "Cortical thickness", "T1w/T2w ratio", "MEG delta", "MEG theta", "MEG alpha", "MEG beta", "MEG gamma1", "MEG gamma2"))
...
gbc_box <- ggplot(data=gbc_22q_nuls_long, aes(y=value, x=variable))+
  geom_boxplot(outlier.shape=21, color="grey50")+
...
rsfa_box <- ggplot(data=rsfa_22q_nuls_long, aes(y=value, x=variable))+
  geom_boxplot(outlier.shape=21, color="grey50")+
  #geom_half_violin(fill="grey50", side="r", lty="blank", alpha=0.5)+
