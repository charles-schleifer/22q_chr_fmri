...
cortex_from_matrix <- function(mat, col1="roi_1", col2="roi_2", variable="pearson_r_Fz", rois=1:360, key=ji_key){
  ...
  out <- filter(dat, col1 %in% rois & col2 %in% rois)[,c("MRI_S_ID","site","col1","col2",variable)]
...
trio_cmat <- lapply(filter(demo_multisite_bl, Site=="UCLAtrio")$MRI_S_ID, function(s) cortex_from_matrix(mat=read_csv_results(sesh=s,site="UCLAtrio",sdir=trio_dir,fname=bparc_name))) %>% do.call(rbind,.)

prisma_cmat <- lapply(filter(demo_multisite_bl, Site=="UCLAprisma")$MRI_S_ID, function(s) cortex_from_matrix(mat=read_csv_results(sesh=s,site="UCLAprisma",sdir=prisma_dir,fname=bparc_name_prisma))) %>% do.call(rbind,.)

suny_cmat <- lapply(filter(demo_multisite_bl, Site=="SUNY")$MRI_S_ID, function(s) cortex_from_matrix(mat=read_csv_results(sesh=s,site="SUNY",sdir=suny_dir,fname=bparc_name))) %>% do.call(rbind,.)

rome_cmat <- lapply(filter(demo_multisite_bl, Site=="Rome")$MRI_S_ID, function(s) cortex_from_matrix(mat=read_csv_results(sesh=s,site="Rome",sdir=rome_dir,fname=bparc_name))) %>% do.call(rbind,.)

iop_cmat <- lapply(filter(demo_multisite_bl, Site=="IoP")$MRI_S_ID, function(s) cortex_from_matrix(mat=read_csv_results(sesh=s,site="IoP",sdir=iop_dir,fname=bparc_name))) %>% do.call(rbind,.)

cmat_all <- rbind(trio_cmat,prisma_cmat,suny_cmat,rome_cmat,iop_cmat)
# cmat_all$INDEX %<>% as.numeric
cmat_all$pearson_r_Fz %<>% as.numeric
cmat_all$INDEX <- paste0(cmat_all$col1, "_", cmat_all$col2)
...
run_neurocombat <- function(input, 
  ...
  # get list of indices
  indices <- unique(input[,index_col])
  ...
  combat_l <- melt.data.table(combat_d, id.vars=index_col) %>% rename("MRI_S_ID"="variable")
  # merge with input data
  out <- merge(x=input, y=combat_l, by=c(index_col, "MRI_S_ID"), all.x=TRUE, all.y=TRUE)
  return(out)
}

# run combat
cmat_combat_22q <- run_neurocombat(input=cmat_all[c("MRI_S_ID","pearson_r_Fz","INDEX")], var_column="pearson_r_Fz")
...
# 
# cmat_norm <- hc_normalize(df=cmat_combat_22q, hcs_ids=hc_ids)
...
cmat_wide <- rows_to_cols(results=cmat_combat_22q, demo=demo_22q_napls, value_var="value")
```
...
cmat_22q_lms <- get_parcel_group_lm(list=cmat_wide, groups=c("22qDel","CONTROL"), refgroup="CONTROL", refsite="UCLAtrio", roi_key=filter(ji_key,INDEX %in% 1:360))
...
if(save_outputs==TRUE){
  save.image(file=file.path(project,"git_exclude/22q_chr_fmri_cmat.Rdata"),compress = "gzip")
}
save_outputs=FALSE
```
