...
packages <- c("devtools","conflicted","here","magrittr", "dplyr", "tidyr","readxl", "ggplot2","gghalves","patchwork", "ggrepel", "ggpubr", "gt", "scico", "RColorBrewer", "ciftiTools", "tableone", "data.table", "reshape2", "neuroCombat", "circlize")
...
# load(file.path(project,"git_exclude/22q_chr_fmri_cmat.Rdata"))
```
...
get_hc_stats_fast <- function(hcs_df, hcs_ids, parc){
  print(parc)
  dfh <- filter(hcs_df, INDEX == parc)
...
hc_normalize_fast <- function(df, hcs_ids){
  print("starting")
  # first get hc stats for all parcels
  inds <- unique(df$INDEX)
  print(inds)
  print("starting hc stats")
  hcs_df <- filter(df, MRI_S_ID %in% hcs_ids)
  hc_stats <- lapply(inds, function(p) get_hc_stats_fast(parc=p, hcs_df=hcs_df, hcs_ids=hcs_ids)) %>% do.call(rbind,.)
  hc_stats$INDEX <- inds
  print("hc stats done")
...
cmat_norm <- hc_normalize_fast(df=cmat_combat_22q, hcs_ids=hc_ids)
# started 4:08pm
...
cmat_wide <- rows_to_cols(results=cmat_norm, demo=demo_22q_napls, value_var="value_normed")
```

test linear models for 22q vs TD 
...
cmat_22q_lms <- get_parcel_group_lm(list=cmat_wide, groups=c("22qDel","CONTROL"), refgroup="CONTROL", refsite="UCLAtrio")
...
get roi info from key
```{r getROIinfo, message=FALSE, warning=FALSE, paged.print=FALSE}
# get column for each ROI
cmat_22q_lms$roi_1 <- cmat_22q_lms$INDEX %>% gsub("_.*","",.) %>% as.numeric
cmat_22q_lms$roi_2 <- cmat_22q_lms$INDEX %>% gsub(".*_","",.) %>% as.numeric

# merge each with key
cmat_22q_key <- merge(x=cmat_22q_lms[,c("roi_1", "roi_2", "group_beta", "group_p")], y=ji_key[,c("INDEX","GLASSERLABELNAME","NETWORK")], by.x="roi_1", by.y="INDEX") %>% rename("label_1"="GLASSERLABELNAME",.) %>% rename("network_1"="NETWORK",.)

cmat_22q_key <- merge(x=cmat_22q_key, y=ji_key[,c("INDEX","GLASSERLABELNAME","NETWORK")], by.x="roi_2", by.y="INDEX") %>% rename("label_2"="GLASSERLABELNAME",.) %>% rename("network_2"="NETWORK",.)
```

get top % of altered edges by beta weight
```{r getTop10, message=FALSE, warning=FALSE, paged.print=FALSE}
cmat_22q_sorted <- cmat_22q_key[order(-abs(cmat_22q_key$group_beta)),]

nrow_01 <- 0.01*(nrow(cmat_22q_sorted))

cmat_22q_01 <- cmat_22q_sorted[1:nrow_01,]

# make column of +1 or -1 for positive or negative connection
cmat_22q_01$connection <- sign(cmat_22q_01$group_beta)

# function to replace network names with abbreviations
net_abbrev <- function(input){
  out <- input %>% gsub("Visual1","Vis1",.) %>% gsub("Visual2","Vis2",.) %>% gsub("Somatomotor","SOM",.) %>% gsub("Cingulo-Opercular","CON",.) %>% gsub("Dorsal-Attention","DAN",.) %>% gsub("Language","Lang",.) %>% gsub("Frontoparietal","FPN",.) %>% gsub("Auditory","Aud",.) %>% gsub("Default","DMN",.) %>% gsub("Posterior-Multimodal","PMN",.) %>% gsub("Ventral-Multimodal","VMN",.) %>% gsub("Orbito-Affective","OAN",.)
  return(out)
}

# function to plot connections with chordDiagram from the circlize package https://jokergoo.github.io/circlize_book/book/introduction.html
net_chord <- function(network, input, path=file.path(project,"figures/chords/")){
  #circos.par(RESET=TRUE)
  circos.par(cell.padding=c(0,0,0,0))
  # get only chosen network
  cmat_net1 <- filter(input, network_1 == network) 
  cmat_net2 <- filter(input, network_2 == network)
  # combine so that chosen network is in the same column 
  cmat_net <- data.frame(start=c(cmat_net1$network_1, cmat_net2$network_2), end=c(cmat_net1$network_2, cmat_net2$network_1), connection=c(cmat_net1$connection, cmat_net2$connection))
  # sort
  cmat_sort <- cmat_net[order(cmat_net$end),]
  # shorten names
  cmat_abr <- cmat_sort
  cmat_abr$start <- net_abbrev(cmat_abr$start)
  cmat_abr$end <- net_abbrev(cmat_abr$end)
  print(paste("rows=",nrow(cmat_abr)))
  # positive connection is red, negative is blue  
  col_fun = colorRamp2(range(-1,1), c("blue", "red"), transparency = 0.2)
  # make chord diagram
  #circos.par(canvas.xlim = c(-1.5, 1.5), canvas.ylim = c(-1.5, 1.5))
  chordDiagram(cmat_abr, col=col_fun, annotationTrack = "grid", grid.col = c("Vis1"="#0000FE", "Vis2"="#783CEC", "SOM"="#4CFCFC", "CON"="#B329B2", "DAN"="#02F100", "Lang"="#018E95", "FPN"="#FFFC3D", "Aud"="#F96BFB", "DMN"="#FF320D", "PMN"="#C5784A", "VMN"="#F3A42B", "OAN"="#5D9A18"), annotationTrackHeight = 0.03, preAllocateTracks = list(1, track.height = 0.2))
  # circos.track(track.index = 1, panel.fun = function(x, y) {
  #     circos.text(CELL_META$xcenter, CELL_META$ylim[1], CELL_META$sector.index, 
  #         facing = "clockwise", niceFacing = TRUE, adj = c(-0.08, 0.5))
  # }, bg.border = NA)
  ...
  dev.copy(png,file.path(path,paste0(network,".png")), width=5, height=5, units="in", res=300)
  dev.off()
}

# plots for each network
for (net in ji_net_keys$NETWORK){
  print(net)
  net_chord(network = net, input=cmat_22q_01)
}
```
